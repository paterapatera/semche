...
- [x] ストーリーの背景と目的を確認する
- - セマンティック検索機能のためのドキュメント登録機能の前段階として、入力文章を埋め込みモデルでベクトル化し出力する機能を作成する。
- [x] 実装する機能の詳細をユーザーと対話して決定する
- - 使用する埋め込みモデルは `sentence-transformers/stsb-xlm-r-multilingual` で確定。
- - 出力形式はLangChain/ChromaDBで登録可能な形（768次元float配列、List[List[float]]形式）で実装。
- - エラーケースはカスタム例外（EmbeddingError等）を送出し、詳細メッセージ・ログ出力でデバッグ容易性を担保。
- [x] 技術スタック（LangChain、Hugging Face、PyTorch）で問題ないことを確認。
- [x] ファイル構成案（`src/semche/embedding.py`、テストは`tests/`、設計書は`.exp.md`）で承認済み。
- [x] **Phase 1完了の確認をユーザーから得て次に進む**
- [x] 承認を得た内容をストーリーに反映する

### Phase 2: 実装【実装フェーズ】

- [x] LangChainとHugging Faceの依存関係がインストールされている
- [x] `sentence-transformers/stsb-xlm-r-multilingual`モデルが正常にロードできる
- [x] 文字列をベクトルデータに変換する機能が実装されている
- [x] 単一文字列の変換が正常に動作する
- [x] 複数文字列のバッチ処理が正常に動作する
- [x] ベクトルデータの出力形式が適切である（配列、次元数など）
